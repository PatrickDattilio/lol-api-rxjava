...
class Team_v2_4 extends RiotApiBase implements RiotApi.Team {
	...
	interface Interface {
		...
		public Call<Map<String,TeamDto[]>> getTeamsBySummoners(@Path("region") Region region, @Path("summonerIds") CSA summonerIdsteamIds, @Query("api_key") ApiKey apiKey);

		@GET("/api/lol/{region}/v2.4/team/by-summoner/{teamIds}")
		public Call<Map<String,TeamDto>> getTeams(@Path("region") Region region, @Path("teamIds") CSA teamIds, @Query("api_key") ApiKey apiKey);
	}
